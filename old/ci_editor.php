...
(
    ...
    {
        $arr = array('action'=>'ci', 'id'=>$this->id, 'task'=>null, 'dependency_id'=>null);
    ...
    function removeWrite()
    {
        ...
        redirect(makeUrl(array('id'=>null, 'action'=>'ci_list', 'task'=>null)));
    }
    ...
    function makePopupForm($field_name, $input, $input_id, $popup_id)
    {
        ...
        return makeForm($form, array('task'=>'updateField','action'=>'ci','id'=>$this->id,'key'=>$field_name));
    }
    ...
    function getActionMenu() 
    {
        ...
        $action_links[] = makeLink("?action=ci&amp;task=create", "Create new item", null, "Creat an empty new CI");
...
order by cl2.create_time desc;', 
    ...
    function historyWrite()
    {
        ...
        setTitle("History for " . $ci->getDescription());
...
";
        ...
        foreach($edits as $edit) {
            ...
            */
            ...
            if (!$noop) {
                $username = makeLink(array('action'=>'user','id'=>$edit['user_id']), $edit['username']);
                $time = date_format($edit['create_time']);
...
";
    ...
    function viewWrite() 
    {
        ...
        setTitle("$action_str " . $ci->getDescription());
        ...
        if ($edit) {
            ...
            $content .= "<input type='hidden' name='action' value='ci'>";
...
";
    ...
    function makeDependencies()
    {
        ...
        if (!$revision) {
            ...
            $content .= makeForm($form, array('action'=>'ci', 'task'=>'addDependency','id'=>$this->id));
            $content .= "</td></tr>";
        }
        ...
        if (!$revision) {
            ...
            $form .= "</select></td><td><button type='submit'>Add</button>\n";
            $content .= makeForm($form, array('action'=>'ci', 'task'=>'addDependant','id'=>$this->id));
