...
";
		...
		$content .= makeForm($form,array('action'=>'ci_list'), 'get');
...
";
		...
		foreach($ci_list as $ci) {
			...
			$content .= makeLink(array('action' => 'ci', 'id' => $ci->id),$ci->getDescription(false), null, "View detailed information for the CI " . $ci->getDescription(false) );
			$content .= "</td><td>";
			$content .= ciType::getName($ci->ci_type_id);
			
			$content .= "</td><td>";
			$content .= date_format($ci->update_time);
			$content .= "</td><td>";

			$content .= makeLink(array('action' => 'ci', 'id' => $ci->id,'task'=>'remove'),'Remove', 'remove', "Remove the CI " . $ci->getDescription());
			$content .= "</td></tr>";
		...
		$this->show(array(makeLink("?action=ci&amp;task=create", "Create new item", null, "Creat an empty new CI")), 
			    $content);
