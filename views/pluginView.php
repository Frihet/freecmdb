...
";
        ...
        foreach(db::fetchList("select * from ci_plugin") as $plugin) {
            $content .= "<li>" . makeLink(array("plugin"=>$plugin['name'], 'controller' => 'configure'), 
                                          $plugin['name'] . " - " . $plugin['description']) . makeLink(array('controller' => 'plugin', 'plugin' => $plugin['name'],'task'=>'uninstall'),'Remove', 'remove', "Remove the Plugin",array('onclick'=>'return confirm("Are you sure?");'))."</li>";
        }
        $content .= "</ul>";
